...
function PeopleCounter({ onPeopleCount, inputRef }) {
    const [error, setError] = useState(false);

    const handlePeopleCounter = (event) => {
    const peopleCount = event.target.value;
    onPeopleCount(Number(peopleCount));

    setError(Number(peopleCount) === 0);
};

    return (
        <section className="people-counter__section">

        <header className='people-counter__header'>
        <h4 className="people-counter__header_text">Number of People</h4>
        {error && (
            <h4 className="people-counter__header_error-msg">Can't be zero</h4>
        )}
        </header>

        <label className='people-counter__label' htmlFor="people-counter"> 
